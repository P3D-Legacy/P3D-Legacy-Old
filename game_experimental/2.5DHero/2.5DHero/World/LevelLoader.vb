...
#Region "File Loading"
    ...
    Private Sub InternalLoad()
        ...
        Logger.Debug("082", "Loading map: " & levelPath.Remove(0, GameController.GamePath.Length))
        Security.FileValidation.CheckFileValid(levelPath, False, "LevelLoader.vb")

        If IO.File.Exists(levelPath) = False Then
            Logger.Log("185", Logger.LogTypes.ErrorMessage, "LevelLoader.vb: Error accessing map file """ & levelPath & """. File not found.")
            Busy -= 1
        ...
        For Each line As String In Data
            If line.Contains("{") = True Then
                ...
                If line.StartsWith("{""Comment""{COM") = True Then
                    ...
                    Logger.Log("186", Logger.LogTypes.Debug, line)
                End If
            End If
        Next
        ...
        For i = 0 To Integer.MaxValue
            ...
            If line.Contains("{") = True And line.Contains("}") = True Then
                ...
                Catch ex As Exception
                    Logger.Log("187", Logger.LogTypes.Warning, "LevelLoader.vb: Failed to load map object! (Index: " & countLines & ") from mapfile: " & levelPath & "; Error message: " & ex.Message)
                End Try
            End If
        Next
        ...
        For Each line As String In Data
            ...
            If line.Contains("{") = True And line.Contains("}") = True Then
                Try
                    ...
                    End If
                Catch ex As Exception
                    Logger.Log("188", Logger.LogTypes.Warning, "LevelLoader.vb: Failed to load map object! (Index: " & countLines & ") (Line: " & orgLine & ") from mapfile: " & levelPath & "; Error message: " & ex.Message)
                End Try
            End If
        Next
        ...
        Logger.Debug("083", "Map loading finished: " & levelPath.Remove(0, GameController.GamePath.Length))
        Logger.Debug("084", "Loaded textures: " & TextureManager.TextureList.Count.ToString())
        timer.Stop()
        Logger.Debug("085", "Map loading time: " & timer.ElapsedTicks & " Ticks; " & timer.ElapsedMilliseconds & " Milliseconds.")
...
#End Region
    ...
    Private Sub AddOffsetMap(ByVal Tags As Dictionary(Of String, Object))
        If Core.GameOptions.LoadOffsetMaps > 0 Then
            ...
            Else
                Logger.Debug("086", "Loaded Offsetmap from store: " & MapName)
                ...
                Next
            End If
            Logger.Debug("087", "Offset maps in store: " & OffsetMaps.Count)
...
#Region "AddElements"
    ...
    Private Function AddStructure(ByVal Tags As Dictionary(Of String, Object)) As String()
        ...
        If tempStructureList.ContainsKey(structureKey) = False Then
            ...
            If IO.File.Exists(filepath) = False Then
                Logger.Log("189", Logger.LogTypes.ErrorMessage, "LevelLoader.vb: Error loading structure from """ & filepath & """. File not found.")
    ...
    Private Sub SetupLevel(ByVal Tags As Dictionary(Of String, Object))
        ...
        If ContentPackManager.GetMusicReplacement(Screen.Level.LevelFile()).ToLower() <> "none" Then
            MusicLoop = ContentPackManager.GetMusicReplacement(Screen.Level.LevelFile())
        End If
