    ...
    Public Class SessionManager
            ...
            If API.LoggedIn = True Then
                Select Case Status
                    Case "Idle"
                        ...
                        End If
                    Case "WaitingForOpen"
                        Logger.Debug("136", "Session started!")
        ...
        Shared Sub Start()
            If API.LoggedIn = True And Status <> "WaitingForCheck" Then
                Logger.Debug("138", "Starting session...")
        ...
        Shared Sub CheckedSession(ByVal result As String)
            ...
            If list(0).Value = "true" Then
                Logger.Log("237", Logger.LogTypes.Warning, "SessionManager.vb: Tried to log in with an already logged in account!")
            End If

            'If list(0).Value = "false" Or DPokemon3D.IS_DEBUG_ACTIVE = True Then
            '    Dim APICall As New APICall()
            '    APICall.OpenSession()
        ...
        Public Shared Sub Close()
            If API.LoggedIn = True Then
                Logger.Debug("139", "Closing session...")
        ...
        Private Shared Sub Kick(ByVal ex As Exception)
            If API.LoggedIn = True Then
                API.LoggedIn = False
                SessionStarted = False
                Status = "Idle"
