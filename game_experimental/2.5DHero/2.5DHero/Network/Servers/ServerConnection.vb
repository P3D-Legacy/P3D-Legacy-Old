...
Namespace Servers
    ...
    Public Class ServerConnection
        ...
        Private Sub InternalConnect(ByVal ServerObject As Object)
            ...
            Try
                ...
                If Me._client.Connected = True Then
                    ...
                    Logger.Debug("118", "Sent initial data to server.")

                    'Start background activities:
                    Me.StartPing()
                    Me.StartListen()
                Else
                    Logger.Log("204", Logger.LogTypes.Warning, "ServerConnection.vb: Server connection error.")
                    Me.Disconnect("Cannot connect to server!", "Connection error!")
                End If
            Catch ex As Exception
                Logger.Log("205", Logger.LogTypes.Warning, "ServerConnection.vb: Server connection exception.")
        ...
        ''' </summary>
        Public Sub Abort()
            Logger.Debug("119", "ServerConnection.vb: Aborting threads and streams...")
            Me._ConnectionOpen = False
            JoinServerScreen.Online = False
            Try
                If Not Me._client Is Nothing Then
                    Me._client.Close()
                    Logger.Debug("120", " - - Closed TCP Client")
                End If
            Catch : End Try
            Try
                Me.StopPing()
            ...
            If Not Me._stream Is Nothing Then
                Try
                    ...
                    Logger.Debug("121", " - - Closed stream reader")
                Catch : End Try
                Try
                    Me._streamWriter.Close()
                    Logger.Debug("122", " - - Closed stream writer")
                Catch : End Try
                Try
                    Me._stream.Close()
                    Logger.Debug("123", " - - Closed stream")
                Catch : End Try
            End If
            Logger.Debug("124", "ServerConnection.vb: Aborted threads and streams.")
        End Sub
        ...
        Public Sub Disconnect(ByVal Header As String, ByVal Message As String)
            If Me._ConnectionOpen = True Then
                Logger.Debug("125", "Disconnect; Header: " & Header & "; Message: " & Message)
...
#Region "Ping"
        ...
        Public Sub StartPing()
            ...
            Logger.Debug("126", "Starting Servers ping thread.")
        End Sub
        ...
        ''' </summary>
        Private Sub InternalPing(sender As Object, e As ElapsedEventArgs)
            Try
        ...
        Private Sub InternalSendPackage(ByVal packageObject As Object)
            ...
            If Me._ConnectionOpen = True And Me._client.Connected = True Then
                ...
                Try
                    ...
                    Me._streamWriter.Flush()
                Catch ex As Exception
                    Logger.Log("206", Logger.LogTypes.Warning, "ServerConnection.vb: Error while sending data to server (TCP): " & ex.Message)
                ...
                Try
                    ...
                    If p.IsValid = True Then
                        Try
                            p.Handle()
                        Catch ex As Exception
                            Logger.Debug("127", "Error trying to handle server data package.")
                        End Try
                    Else
                        Logger.Debug("128", "Received invalid server package.")
                    End If
                Catch ex As Exception
                    Logger.Log("207", Logger.LogTypes.Warning, "ServerConnection.vb: Error while receiving server data (TCP): " & vbNewLine & ex.Message)
        ...
        Private Sub StopListen()
            Try
                If Not Me._receiveThread Is Nothing Then
                    Me._receiveThread.Abort()
                    Logger.Debug("129", " - - Aborted listen thread")
                End If
            Catch : End Try
        End Sub
