Public Class ChatScreen
    ...
    Public Overrides Sub Update()
        ...
        If Controls.Dismiss(True, False) = True Or (JoinServerScreen.Online = False And Core.Player.SandBoxMode = False And GameController.IS_DEBUG_ACTIVE = False) = True Then
            Core.SetScreen(Me.PreScreen)
        Else
            If KeyBoardHandler.KeyPressed(KeyBindings.EnterKey1) = True Or ControllerHandler.ButtonPressed(Buttons.A) = True Then
    ...
    Private Sub Copy()
        If Me.SelectionLength > 0 Then
            ...
            Catch ex As Exception
                Logger.Log("255", Logger.LogTypes.Warning, "ChatScreen.vb: An error occurred while copying text to the clipboard (""" & Me.currentText.Substring(Me.SelectionStart, Me.SelectionLength) & """).")
            End Try
        End If
    End Sub
    ...
    Private Sub Cut()
        If Me.SelectionLength > 0 Then
            Try
                ...
                SelectionLength = 0
            Catch ex As Exception
                Logger.Log("256", Logger.LogTypes.Warning, "ChatScreen.vb: An error occurred while Cutting text to the clipboard (""" & Me.currentText.Substring(Me.SelectionStart, Me.SelectionLength) & """).")
            End Try
        End If
    End Sub
