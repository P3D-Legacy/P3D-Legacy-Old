...
Public Class PressStartScreen
    ...
    Private Sub ChangeLevel()
        If _mainMenuMaps Is Nothing Then
            ...
            _mainMenuMaps = DataModel.Json.JsonDataModel.FromString(Of DataModel.Json.Game.MainMenuMapModel())(jsonFileContent)
        End If

        Dim levelID As Integer = Random.Next(0, _mainMenuMaps.Length)

        Camera.Position = _mainMenuMaps(levelID).Offset.ToVector3()
        Level.Load(_mainMenuMaps(levelID).Mapfile)
    End Sub
        ...
        Public Sub SelectProfile()
            ...
            Else
                If _isGameJolt And _loaded = False And GameJolt.API.LoggedIn = False Then
                    SetScreen(New GameJolt.NewLogInScreen(CurrentScreen))
                Else
