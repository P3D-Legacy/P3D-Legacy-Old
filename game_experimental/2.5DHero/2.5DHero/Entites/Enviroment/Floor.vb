Public Class Floor
    ...
    Public Overrides Sub Render()
        If changedWeatherTexture = False Then
            ...
            If (Screen.Level.World.CurrentMapWeather = Pokemon3D.World.Weathers.Snow Or Screen.Level.World.CurrentMapWeather = Pokemon3D.World.Weathers.Blizzard) = True And Me.hasSnow = True Then
                ChangeSnow()
            End If
            If Screen.Level.World.CurrentMapWeather = Pokemon3D.World.Weathers.Sandstorm And Me.hasSand = True Then
                ChangeSand()
            End If
        End If
    ...
    Private Sub ChangeSnow()
        ...
        If Core.CurrentScreen.Identification = Screen.Identifications.BattleScreen Then
            Me.Textures(0) = Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(208, 16, 16, 16))
            'Me.Position.Y += 0.05F
        Else
            ...
            If hasEntityOnAllSides = False Then
                Me.Textures = {Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(208, 16, 16, 2)), Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(208, 16, 16, 16))}
    ...
    Private Sub ChangeSand()
        Me.Rotation = New Vector3(Me.Rotation.X, 0.0F, Me.Rotation.Z)
        If Core.CurrentScreen.Identification = Screen.Identifications.BattleScreen Then
            Me.Textures(0) = Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(240, 80, 16, 16))
            'Me.Position.Y += 0.05F
        Else
            Dim hasEntityOnAllSides As Boolean = True
            Dim ent(4) As Entity
            ...
            If hasEntityOnAllSides = False Then
                Me.Textures = {Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(240, 80, 16, 2)), Pokemon3D.TextureManager.GetTexture("Routes", New Rectangle(240, 80, 16, 16))}
                Me.Model = BaseModel.BlockModel
                Me.TextureIndex = {sides(0), sides(0), sides(1), sides(1), sides(2), sides(2), sides(3), sides(3), 1, 1}
                Me.Scale = New Vector3(1, 0.1F, 1)
                Me.Position.Y -= 0.45F
