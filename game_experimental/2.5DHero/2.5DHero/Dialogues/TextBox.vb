Public Class TextBox
    ...
    Private _arrowAlpha As Integer = 0
    Private _arrowAlphaGrowing As Boolean = True
    ...
    Public Sub Update()
        If Showing = True Then
            If _arrowAlphaGrowing Then
                _arrowAlpha += 5
                If _arrowAlpha >= 255 Then
                    _arrowAlphaGrowing = False
                    _arrowAlpha = 255
                End If
            Else
                _arrowAlpha -= 5
                If _arrowAlpha <= 0 Then
                    _arrowAlphaGrowing = True
                    _arrowAlpha = 0
                End If
            End If
    ...
    Public Sub Draw()
        If Me.Showing = True Then
            With Core.SpriteBatch
                .Draw(Content.Load(Of Texture2D)("SharedResources\Textures\UI\Overworld\main"), New Rectangle(CInt(Core.windowSize.Width / 2) - 240, CInt(PositionY), 480, 144), New Rectangle(0, 48, 160, 48), Color.White)
                ...
                If Me.CanProceed = True And Me.through = True Then
                    .Draw(Content.Load(Of Texture2D)("SharedResources\Textures\UI\Overworld\main"), New Rectangle(CInt(Core.windowSize.Width / 2) + 190, CInt(PositionY) + 100, 32, 32), New Rectangle(96, 16, 32, 32), Color.White)
                    .Draw(Content.Load(Of Texture2D)("SharedResources\Textures\UI\Overworld\main"), New Rectangle(CInt(Core.windowSize.Width / 2) + 190, CInt(PositionY) + 100, 32, 32), New Rectangle(128, 16, 32, 32), New Color(255, 255, 255, _arrowAlpha))
                End If
            End With
        End If
    End Sub
