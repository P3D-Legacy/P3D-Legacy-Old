Namespace Security
    ...
    Public Class FileValidation
        ...
        Const EXPECTEDSIZE As Integer = 40178016
        Const METAHASH As String = "NkVFNUIzQTg2RTM5RUM2ODAyRUNDOTE4MkUzQ0M3MDM="
        ...
        Private Shared Function FilesValid() As Boolean
            ...
            Dim includeExt() As String = {".dat", ".poke", ".trainer"}

            If RUNVALIDATION = True Then
                Logger.Log("208", Logger.LogTypes.Debug, "FileValidation.vb: WARNING! FILE VALIDATION IS RUNNING!")
                For Each file As String In IO.Directory.GetFiles(GameController.GamePath & "\GameModes\Kolben", "*.*", IO.SearchOption.AllDirectories)
                ...
                For Each f As String In files
                    Dim i As Long = New IO.FileInfo(GameController.GamePath & "\" & f).Length
                ...
                IO.File.WriteAllText(GameController.GamePath & "\meta", s)
                Logger.Log("209", Logger.LogTypes.Debug, "FileValidation.vb: Meta created! Expected Size: " & MeasuredSize &
                           "|MetaHash: " & StringObfuscation.Obfuscate(GetMD5FromFile(GameController.GamePath & "\meta")))

                GameInstance.Exit()
            Else
                If IO.File.Exists(GameController.GamePath & "\meta") = True Then
                    If GetMD5FromFile(GameController.GamePath & "\meta") = StringObfuscation.DeObfuscate(METAHASH) Then
                        files = IO.File.ReadAllText(GameController.GamePath & "\meta").Split(CChar(",")).ToList()
                        Logger.Debug("148", "Meta loaded. Files to check: " & files.Count)
                    Else
                        Logger.Log("210", Logger.LogTypes.Warning, "FileValidation.vb: Failed to load Meta (Hash incorrect)! File Validation will fail!")
                    End If
                Else
                    Logger.Log("211", Logger.LogTypes.Warning, "FileValidation.vb: Failed to load Meta (File not found)! File Validation will fail!")
                End If
                ...
                For Each f As String In files
                    ...
                    If IO.File.Exists(GameController.GamePath & "\" & fileName) Then
                        Dim i As Long = New IO.FileInfo(GameController.GamePath & "\" & fileName).Length
        ...
        Public Shared Sub CheckFileValid(ByVal file As String, ByVal relativePath As Boolean, ByVal origin As String)
            ...
            If validationResult <> "" Then
                Logger.Log("212", Logger.LogTypes.ErrorMessage, "FileValidation.vb: Detected invalid files in a sensitive game environment. Stopping execution...")
        ...
        Private Shared Function ValidateFile(ByVal file As String, ByVal relativePath As Boolean) As String
            If Core.Player.IsGameJoltSave = True And GameController.IS_DEBUG_ACTIVE = False Then
                ...
                End If
                Dim i As Long = New IO.FileInfo(filePath).Length
