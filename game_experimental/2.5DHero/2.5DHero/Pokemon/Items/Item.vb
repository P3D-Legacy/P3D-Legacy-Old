...
#Region "Fields"
    ...
    Private _loadedTexture As Boolean = False
...
#Region "Properties"
    ...
    Public ReadOnly Property BattleSelectPokemon() As Boolean
        Get
            Return _requiresPokemonSelectInBattle
        End Get
    End Property
...
#End Region
    ...
    Protected Sub Initialize(ByVal Name As String, ByVal Price As Integer, ByVal ItemType As ItemTypes, ByVal ID As Integer, ByVal CatchMultiplier As Single, ByVal SortValue As Integer, ByVal TextureRectangle As Rectangle, ByVal Description As String)
        _name = Name
        _pluralName = Name & "s" 'Default plural name with "s" at the end.
        _pokeDollarPrice = Price
        _itemType = ItemType
        _id = ID
        ...
        If ItemNameID.ContainsKey(_name) = False Then
            ItemNameID.Add(_name, _id)
        End If
    ...
    ''' <summary>
    ''' Loads the texture of the item.
    ''' </summary>
    Protected Overridable Sub LoadTexture()
        _texture = TextureManager.GetTexture("Items\ItemSheet", _textureRectangle, "")
    End Sub
    ...
    Public Overridable Sub Use()
        Logger.Debug("168", "PLACEHOLDER FOR ITEM USE")
    End Sub
    ...
    Public Overridable Function UseOnPokemon(ByVal PokeIndex As Integer) As Boolean
        ...
        Logger.Debug("169", "PLACEHOLDER FOR ITEM USE ON POKEMON")
        Return False
    End Function

    ''' <summary>
    ''' Opens the selection screen for an item usage.
    ''' </summary>
    Protected Sub UseOnPokemonSelect()
        Dim selScreen As New PartyScreen(Core.CurrentScreen)
        selScreen.Mode = Screens.UI.ISelectionScreen.ScreenMode.Selection
        selScreen.CanExit = True

        AddHandler selScreen.SelectedObject, AddressOf UseOnPokemonHandler
        Core.SetScreen(selScreen)
    End Sub

    Private Sub UseOnPokemonHandler(ByVal params As Object())
    ...
    Public Function RemoveItem() As String
        ...
        If Core.Player.Inventory.GetItemAmount(ID) = 0 Then
            Return "*There are no~" & PluralName & " left." 'MASS EXTINCION
        End If
        Return ""
    End Function
    ...
    Public Shared Function GetItemByID(ByVal ID As Integer) As Item
        ...
        Select Case ID
            ...
            Case 286
                Return New Items.Standard.RockIncense()
            Case 287
                Return New Items.Standard.RoseIncense()
            Case 288
                Return New Items.Standard.FullIncense()
            Case 289
                Return New Items.Standard.LaxIncense()
            Case 290
    ...
    Public Shared Function GetItemByName(ByVal name As String) As Item
        ...
        If ItemNameID.Keys.Contains(name.ToLower()) = True Then
            Return GetItemByID(ItemNameID(name.ToLower()))
        Else
            For i = 0 To maxID
            ...
            Next
        End If
        Logger.Log("262", Logger.LogTypes.Warning, "Item.vb: Cannot find item with the name """ & name & """.")
        Return Nothing
    End Function
    ...
    ''' <summary>
    ''' Converts an item type to its proper display representation.
    ''' </summary>
    ''' <param name="itemType">The ItemType to convert.</param>
    Public Shared Function GetItemTypeName(ByVal itemType As ItemTypes) As String
        Select Case itemType
            Case ItemTypes.Standard, ItemTypes.Machines, ItemTypes.Mail, ItemTypes.Medicine, ItemTypes.Plants
                Return itemType.ToString()
            Case ItemTypes.BattleItems
                Return "Battle Items"
            Case ItemTypes.KeyItems
                Return "Key Items"
            Case ItemTypes.Pokéballs
                Return "Poké Balls"
        End Select
