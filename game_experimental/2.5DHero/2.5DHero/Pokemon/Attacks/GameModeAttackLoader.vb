Namespace BattleSystem
    ...
    Public Class GameModeAttackLoader
        ...
        'The default relative path to load moves from.
        Const PATH As String = "Moves\"
        ...
        Public Shared Sub Load()
            ...
            If GameModeManager.ActiveGameMode.IsDefaultGamemode = False Then
                For Each file As String In System.IO.Directory.GetFiles(GameController.GamePath & "\" & GameModeManager.ActiveGameMode.DataPath & "\" & PATH, "*.dat")
                    LoadMove(file)
                Next
            End If

            If LoadedMoves.Count > 0 Then
                Logger.Debug("111", "Loaded " & LoadedMoves.Count.ToString() & " GameMode move(s).")
            End If
        End Sub
        ...
        Private Shared Sub LoadMove(ByVal file As String)
            ...
            Catch ex As Exception
                ...
                Logger.Log("192", Logger.LogTypes.ErrorMessage, "GameModeAttackLoader.vb: Error loading GameMode move from file """ & file & """: " & ex.Message & "; Last Key/Value pair successfully loaded: " & key & "|" & value)
            End Try
            ...
            If setID = True Then
                If move.ID >= 1000 Then
                    ...
                    Else
                        Logger.Log("193", Logger.LogTypes.ErrorMessage, "GameModeAttackLoader.vb: User defined moves are not allowed to have an ID of an already existing move or an ID below 1000. The ID for the move loaded from """ & file & """ has the ID " & move.ID.ToString() & ", which is the ID of an already existing move (" & testMove.Name & ").")
                    End If
                Else
                    Logger.Log("194", Logger.LogTypes.ErrorMessage, "GameModeAttackLoader.vb: User defined moves are not allowed to have an ID of an already existing move or an ID below 1000. The ID for the move loaded from """ & file & """ has the ID " & move.ID.ToString() & ", which is smaller than 1000.")
                End If
            Else
                Logger.Log("195", Logger.LogTypes.ErrorMessage, "GameModeAttackLoader.vb: User defined moves must set their ID through the ""ID"" property, however the move loaded from """ & file & """ has no ID set so it will be ignored.")
            End If
        End Sub
